...
- [ENH] Refactored series_length and n_cases to n_timepoints and n_cases ({pr}`1163`) {user}`Raya679`
...
- [ENH] Refactored series_length and n_cases to n_timepoints and n_cases in dictionary based classifiers #1151 ({pr}`1203`) {user}`Vedant222`
...
- [ENH] Refactored n_cases to n_cases and series_length to n_timepoints ({pr}`1166`) {user}`Raya679`
