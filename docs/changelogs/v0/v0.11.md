...
- [ENH] Unified ROCKET interface with device parameter for CPU/GPU selection - `Rocket` now supports `device` parameter ("cpu", "gpu", or "auto") providing a single interface for both CPU and GPU execution, with automatic GPU detection and fallback capabilities
