...
- New native implementations of estimators, including: ROCKAD anomaly detector;
  K-Shape clusterer; TS-QUAD configuration for in ElasticEnsemble classifier;
  ETS/AutoETS and DeepARF forecasters; and STL, MSTL and LOWESS transformer,
- All data now loaded from zenodo, including from the multiverse, a relaunched
  multivariate classification archive

thanks to our 28 contributors of 85 pull requests since the last release.
...
- [BUG] Fix #3203 - Use proper file names in load_regression method  ({pr}`3204`) {user}
  `Lucky-Lodhi2004`
...
- [MNT] Update dependencies and configuration in pyproject.toml ({pr}`3167`) {user}
  `TUSHARDHOKRIYA`
- [DOC] Fix typos and enhance clarity in getting_started.md ({pr}`3134`) {user}`PoojasPatel013`


## Contributors
The following have contributed to this release through a collective 85 GitHub Pull Requests:
