...
<span style="color:red">**If you input a 2D numpy array, it will be treated as a single multivariate series**</span>.
...
summary time series features from each series, and fit our classifier or regressor on
these features. Time series input are assumed to be in numpy3D format. For example,
we could use catch22, which calculates 22 summary statistics for each series (link).

```{code-block} python
>>> from aeon.transformations.panel.catch22 import Catch22
>>> import numpy as np
>>> # 4 time series of length 10
>>> X1 = np.random.random(size=(4, 1, 10))
>>> c22 = Catch22(replace_nans=True)
>>> X2 = c22.fit_transform(X1)
>>> # transformed into 4 cases with 22 features
```

If the transformer produces vector output, (i.e. 2 dimensional numpy), we can use a
standard scikit classifier or a sckit pipeline.
