...
# Import all shared helpers from TAR (adjust import path if not packaged together)
# If these files live side-by-side, `from _tar import ...` also works.
...
class AutoTAR(BaseForecaster, IterativeForecastingMixin):
    ...
    def _fit(self, y: np.ndarray, exog: np.ndarray | None = None) -> AutoTAR:
