"""NLinearForecaster module for deep learning forecasting in aeon."""

from __future__ import annotations

__maintainer__ = []

__all__ = ["NLinearForecaster"]
...
class NLinearForecaster(BaseDeepForecaster, SeriesToSeriesForecastingMixin):
    ...
    Normalizes distribution shift in data by subtracting the last value of window
    from input and adding it back before last prediction.
    ...
    def build_model(self, input_shape):
        """Build one-layered NLinear model for forecasting.
        ...
        model : tf.keras.Model
            Compiled Keras model with NLinear architecture.
        """
        ...
        network = MLPNetwork(
            n_layers=1,
            n_units=self.horizon * n_channels,
            activation="linear",
            dropout_rate=0.0,
            dropout_last=0.0,
        )
        ...
        x = layers.Reshape((self.horizon, n_channels))(linear_preds)
        outputs = layers.Add(name="denormalization")([x, x_last])
    ...
    def _fit(self, y, exog=None):
        """
        Fit the NLinear Forecaster model to the training data.
    ...
    def _series_to_series_forecast(
        ...
        if exog is not None:
            raise NotImplementedError(
                "Exogenous variables not supported in NLinearForecaster."
            )
