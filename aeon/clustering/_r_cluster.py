...
class RCluster(BaseClusterer):
    ...
    def _fit(self, X, y=None):
        ...
        self.__optimal_dimensions = np.argmax(pca.explained_variance_ratio_ < 0.01)
        self.__optimal_dimensions = min(self.__optimal_dimensions, X.shape[0], X.shape[1])
        if(self.__optimal_dimensions==0):
            raise ValueError(
                f"Optimal dimensions {self.__optimal_dimensions} must be greater than 0."
            )
