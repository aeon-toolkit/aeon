...
__maintainer__ = []
__all__ = ["TimeSeriesAgglomerative"]
...
class TimeSeriesAgglomerative(BaseClusterer):
    ...
    Agglomerative clustering is transductive: it can only cluster the
    collection it is fitted on and does not support out-of-sample prediction.
    Use ``fit_predict(X)`` to cluster a collection, or inspect ``labels_``
    after ``fit(X)``. Calling ``predict`` raises a ``NotImplementedError``.
    ...
    def _predict(self, X):
        ...
        for the data seen during fit. This method is unreachable through the
        public API as the ``capability:predict`` tag is False, but is required
        by the base class.
        """
    ...
    def _get_test_params(cls, parameter_set="default"):
        ...
        """
        return {"n_clusters": 2, "distance": "euclidean", "linkage": "average"}
