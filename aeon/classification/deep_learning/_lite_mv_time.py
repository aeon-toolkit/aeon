...
__all__ = ["LITE_MVTimeClassifier", "IndividualLITE_MVClassifier"]
...
from aeon.networks import LITE_MVNetwork


class LITE_MVTimeClassifier(BaseClassifier):
    """LITE_MVTime ensemble classifier.

    Ensemble of IndividualLITE_MVTimeClassifier objects, as described in [1]_.
    ...
    Examples
    --------
    >>> from aeon.classification.deep_learning import LITE_MVTimeClassifier
    >>> from aeon.datasets import load_unit_test
    >>> X_train, y_train = load_unit_test(split="train")
    >>> X_test, y_test = load_unit_test(split="test")
    >>> lmvtime = LITE_MVTimeClassifier(n_epochs=20,batch_size=4)  # doctest: +SKIP
    >>> lmvtime.fit(X_train, y_train)  # doctest: +SKIP
    ...
    def _fit(self, X, y):
        """Fit the ensemble of IndividualLITE_MVClassifier models.
        ...
        for n in range(0, self.n_classifiers):
            cls = IndividualLITE_MVClassifier(
    ...
    def _predict(self, X) -> np.ndarray:
        """Predict the labels of the test set using LITE_MVTime.
...
class IndividualLITE_MVClassifier(BaseDeepClassifier):
    """Single LITE_MV classifier.

    One LITE Multivariate deep model, as described in [1]_.
    ...
    Examples
    --------
    >>> from aeon.classification.deep_learning import IndividualLITE_MVClassifier
    >>> from aeon.datasets import load_unit_test
    >>> X_train, y_train = load_unit_test(split="train", return_X_y=True)
    >>> X_test, y_test = load_unit_test(split="test", return_X_y=True)
    >>> litemv = IndividualLITE_MVClassifier(n_epochs=20,batch_size=4)  # doctest: +SKIP
    >>> litemv.fit(X_train, y_train)  # doctest: +SKIP
