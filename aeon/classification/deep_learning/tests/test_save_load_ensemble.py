"""Tests for save/load functionality of ensemble deep learning classifiers."""
...
def test_save_load_litetim():
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        model = LITETimeClassifier(n_epochs=1, random_state=42, save_best_model=True, file_path=temp_dir)
        model.fit(X, y)
        ...
        loaded_model = LITETimeClassifier.load_model(model_path=model_files, classes=model.classes_)
...
def test_save_load_inceptiontime():
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        model = InceptionTimeClassifier(n_epochs=1, random_state=42, save_best_model=True, file_path=temp_dir)
        model.fit(X, y)
        ...
        loaded_model = InceptionTimeClassifier.load_model(model_path=model_file, classes=model.classes_)
