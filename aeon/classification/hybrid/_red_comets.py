...
class REDCOMETS(BaseClassifier):
    ...
    References
    ----------
    .. [1] Luca A. Bennett and Zahraa S. Abdallah, "RED CoMETS:"
    ...
    def __init__(
        ...
        _check_soft_dependencies("imbalanced-learn",
    ...
    def _fit_univariate(self, X, y):
        ...
        n_lenses = 2*int(self.perc_length*X.shape[1]//100)
        ...
        if min_neighbours == max_neighbours:
            ...
            if min_neighbours > 5:
                min_neighbours = 6
            try:
                X_smote, y_smote = SMOTE(sampling_strategy="all",
                                         k_neighbors=min_neighbours-1,
                                         random_state=self.rng,
                                         n_jobs=self.n_jobs
                                         ).fit_resample(X, y)
            except ValueError:
                X_smote, y_smote = RandomOverSampler(sampling_strategy='all',
                                                     random_state=self.rng
                                                     ).fit_resample(X, y)
        ...
        self.sfa_transforms = [SFA(word_length=w, alphabet_size=a,
                                   window_size=X_smote.shape[1],
                                   binning_method='equi-width',
                                   save_words=True, n_jobs=self.n_jobs,
                                   random_state=self.rng)
                               for w, a in sfa_lenses]

        for sfa in self.sfa_transforms:
            sfa.fit_transform(X_smote, y_smote)
            def get_word(word): sfa.word_list(int(word[0]))
            X_sfa = np.array(list(map(get_word, sfa.words)))

            rf = RandomForestClassifier(n_estimators=self.n_trees,
                                        random_state=self.rng,
                                        n_jobs=self.n_jobs)
            rf.fit(X_sfa, y_smote)
            weight = cross_val_score(rf, X_sfa, y_smote, cv=cv,
                                     n_jobs=self.n_jobs).mean()

            self.sfa_clfs.append((rf, weight))

        self.sax_transforms = [SAX(n_segments=w, alphabet_size=a)
                               for w, a in sax_lenses]
        ...
        for sax in self.sax_transforms:
            ...
            weight = cross_val_score(rf, X_sax, y_smote, cv=cv,
                                     n_jobs=self.n_jobs).mean()
    ...
    def _predict(self, X) -> np.ndarray:
        ...
        return [self.classes_[i] for i in self._predict_proba(X).argmax(axis=1)]
    ...
    def _get_random_lenses(self, X, n_lenses):
        ...
        lenses = np.array(list(zip(self.rng.choice(n_segments, size=n_lenses),
                                   self.rng.choice(alphas, size=n_lenses))))
        return lenses
