    ...
    for idx in prange(n_jobs):
        ...
        for order in np.arange(start, end):
            ...
            if dist.shape[0] >= k:
                ...
                # fewer than k candidate neighbours exist: cycle through the
                # available indices to fill the row without going out of bounds
                n_avail = dist.shape[0]
                for j in range(k):
                    knns[order, j] = j % n_avail
