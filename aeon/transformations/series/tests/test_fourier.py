...
from aeon.transformations.series import FourierFeaturesTransformer
...
def test_fourier_list_length_missmatch():
    ...
    with pytest.raises(ValueError) as ex:
        FourierFeaturesTransformer(sp_list=[365, 52], fourier_terms_list=[1])
        assert ex.value == (
...
def test_fourier_k_larger_than_sp():
    ...
    with pytest.raises(ValueError) as ex:
        FourierFeaturesTransformer(sp_list=[2], fourier_terms_list=[3])
        assert ex.value == (
...
def test_fourier_redundant_terms_dropped():
    ...
    transformer = FourierFeaturesTransformer(
        sp_list=[12, 9, 3], fourier_terms_list=[4, 3, 1]
    )
    transformer.fit(Y)
...
def test_fit_transform_outputs():
    ...
    y = Y.iloc[:3]
    y_transformed = FourierFeaturesTransformer(
