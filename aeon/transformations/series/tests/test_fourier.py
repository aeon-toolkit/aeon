...
def test_fourier_list_length_missmatch():
    ...
    with pytest.raises(ValueError) as ex:
        FourierSeriesFeatures(sp_list=[365, 52], fourier_terms_list=[1])
        assert ex.value == (
...
def test_fourier_k_larger_than_sp():
    ...
    with pytest.raises(ValueError) as ex:
        FourierSeriesFeatures(sp_list=[2], fourier_terms_list=[3])
        assert ex.value == (
...
def test_fourier_redundant_terms_dropped():
    ...
    transformer = FourierSeriesFeatures(
...
def test_fit_transform_outputs():
    ...
    y = Y.iloc[:3]
    y_transformed = FourierSeriesFeatures(
