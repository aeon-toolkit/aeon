...
def test_clasp_f1_scoring_metric():
    """Test ClaSP with the F1 scoring metric instead of the ROC_AUC default."""
...
def test_clasp_invalid_scoring_metric_raises():
    """Test an unrecognised scoring_metric raises a ValueError."""
    series = np.arange(100, dtype=np.float64)
...
def test_clasp_warns_when_window_too_large():
    """Test a warning is raised when window_length is large relative to series."""
...
def test_clasp_without_interpolation():
    """Test the clasp function with interpolate=False."""
...
def test_sliding_dot_product_handles_odd_lengths():
    """Test sliding dot product with odd-length query and time series."""
...
def test_compute_distances_iterative_pads_when_fewer_than_k_candidates():
    """Test the knn fallback pads with valid indices when fewer than k exist."""
...
def test_binary_f1_score_handles_degenerate_split():
    """Test F1 score does not crash when a class has no predicted positives."""
...
def test_roc_auc_score_degenerate_single_class_is_nan():
    """Test ROC-AUC is nan when only one class is present in y_true."""
