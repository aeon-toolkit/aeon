...
def test_channel_selectors(trans):
    ...
    if issubclass(trans, BaseChannelSelector):
        ...
        # Handle initialization for transformers that need arguments like ChannelScorer
        if trans.__name__ == "ChannelScorer":
            estimator = DummyClassifier()  # Provide the required estimator
            cs = trans(estimator=estimator)  # Initialize ChannelScorer with estimator
        else:
            cs = trans()
