...
class ROCKETGPU(BaseROCKETGPU):
    ...
    def _define_parameters(self):
        ...
        Matches CPU implementation for cross-platform reproducibility.
        Uses MT19937 RNG to ensure identical kernel generation between
        CPU and GPU when using the same random_state.
        """
        # Use same RNG as CPU (MT19937) for reproducibility
        if self.random_state is not None:
            np.random.seed(self.random_state)
            # Also set TensorFlow seed for deterministic operations
            import tensorflow as tf
        ...
        # Generate all kernel lengths first to maintain RNG sequence
        _kernel_lengths = np.random.choice(self._kernel_size, self.n_kernels).astype(
            np.int32
        )

        # Pre-generate number of channels for each kernel
        _num_channels_list = []
        ...
        # Generate weights, channels, biases, dilations, and padding
        for i in range(self.n_kernels):
            ...
            # Select random channels (don't sort to maintain order)
            ...
            # Reshape to TensorFlow format and maintain float32 dtype
            ...
            # Randomly choose padding (translates to SAME or VALID in TensorFlow)
