...
class SAX(BaseCollectionTransformer):
    ...
    znormalize : bool, default = True,
        Whether to z-normalize each series before PAA when ``window_size=None``.
        When windowing is enabled, each extracted window is normalized independently
    ...
    def __init__(
        ...
        if znormalized != "deprecated":
            warnings.warn(
                "The 'znormalized' parameter of SAX is deprecated and will be "
                "removed in v1.7.0. Please use 'znormalize' instead.",
                FutureWarning,
                stacklevel=2,
            )
            self.znormalize = not znormalized
