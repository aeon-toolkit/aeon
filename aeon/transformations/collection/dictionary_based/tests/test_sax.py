...
def test_sax_znormalize_false_matches_manual_normalization():
...
def test_sax_znormalize_false_handles_constant_series():
...
def test_standard_inverse_requires_original_length():
    ...
    sax = SAX(n_segments=4, alphabet_size=4, znormalize=True)
...
def test_unsupported_distribution_raises():
    ...
    with pytest.raises(NotImplementedError):
        SAX(distribution="bogus", znormalize=True)
