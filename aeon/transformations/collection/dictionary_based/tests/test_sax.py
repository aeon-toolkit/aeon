...
def test_sax_default_znormalize_is_true():
    """Test that SAX performs z-normalization by default."""
    sax = SAX()
    assert sax.znormalize is True
    assert sax.znormalized == "deprecated"


def test_sax_znormalized_deprecated():
    """Test that using znormalized emits a FutureWarning and maps to znormalize."""
    with pytest.warns(FutureWarning, match="znormalized"):
        sax_true = SAX(znormalized=True)
    assert sax_true.znormalize is False
    assert sax_true.znormalized is True

    with pytest.warns(FutureWarning, match="znormalized"):
        sax_false = SAX(znormalized=False)
    assert sax_false.znormalize is True
    assert sax_false.znormalized is False
