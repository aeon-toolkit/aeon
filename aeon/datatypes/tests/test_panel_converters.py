...
def test_are_columns_nested(n_instances, n_channels, n_timepoints):
    """Test are_columns_nested for correctness."""
    nested, _ = make_example_nested_dataframe(n_instances, n_channels, n_timepoints)
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_nested_to_3d_numpy(n_instances, n_channels, n_timepoints):
    ...
    assert array.shape == (n_instances, n_channels, n_timepoints)
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_3d_numpy_to_nested(n_instances, n_channels, n_timepoints):
    """Test from_3d_numpy_to_nested for correctness."""
    array = np.random.normal(size=(n_instances, n_channels, n_timepoints))
    nested = from_3d_numpy_to_nested(array)

    # check types and shapes
    assert is_nested_dataframe(nested)
    assert nested.shape == (n_instances, n_channels)
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_nested_to_2d_array(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_3d_numpy_to_2d_array(n_instances, n_channels, n_timepoints):
    """Test from_3d_numpy_to_2d_array for correctness."""
    array = np.random.normal(size=(n_instances, n_channels, n_timepoints))
    array_2d = from_3d_numpy_to_2d_array(array)

    assert array_2d.shape == (n_instances, n_channels * n_timepoints)


@pytest.mark.parametrize("n_instances", N_INSTANCES)
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_multi_index_to_3d_numpy(n_instances, n_channels, n_timepoints):
    """Test from_multi_index_to_3d_numpy for correctness."""
    mi_df = make_example_multi_index_dataframe(
        n_instances=n_instances, n_timepoints=n_timepoints, n_channels=n_channels
    )
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_3d_numpy_to_multi_index(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_multi_index_to_nested(n_instances, n_channels, n_timepoints):
    ...
    assert nested_df.shape == (n_instances, n_channels)
...
def test_from_nested_to_multi_index(n_instances, n_channels, n_timepoints):
    ...
    assert isinstance(mi_df, pd.DataFrame)
    assert mi_df.shape == (n_instances * n_timepoints, n_channels)
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_is_nested_dataframe(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_2d_array_to_nested(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_long_to_nested(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_nested_to_long(n_instances, n_channels, n_timepoints):
    ...
    assert X_long.shape == (n_instances * n_timepoints * n_channels, 4)
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_multiindex_to_listdataset(n_instances, n_channels, n_timepoints):
    ...
    def _make_example_multiindex(
        n_instances, n_channels, n_timepoints, random_seed=42
    ) -> pd.DataFrame:
        import numpy as np
        ...
        inputDF = np.random.randint(
            1, 99, size=(n_instances * n_timepoints, n_channels)
        )
    ...
    MULTIINDEX_DF = _make_example_multiindex(n_instances, n_channels, n_timepoints)
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_3d_numpy_to_nplist(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_dflist_to_nplist(n_instances, n_channels, n_timepoints):
...
@pytest.mark.parametrize("n_channels", N_CHANNELS)
@pytest.mark.parametrize("n_timepoints", N_TIMEPOINTS)
def test_from_nested_to_nplist(n_instances, n_channels, n_timepoints):
