...
def naive_distance_profile(X, q, mask, distance_function, distance_args=None):
    ...
    mask : array, shape (n_channels, n_timepoints - query_length + 1)
...
def normalized_naive_distance_profile(
    ...
    mask : array, shape (n_channels, n_timepoints - query_length + 1)
        Boolean mask of the shape of the distance profile indicating for which part
        of it the distance should be computed.
    X_means : array, shape (n_channels, n_timepoints - query_length + 1)
        Means of each subsequences of X of size query_length
    X_stds : array, shape (n_channels, n_timepoints - query_length + 1)
...
def _naive_distance_profile(
    ...
    distance_profile = np.full((n_channels, profile_size), np.inf)

    for i_candidate in range(profile_size):
        if mask[i_candidate]:
            for i_channel in range(n_channels):
                distance_profile[i_channel, i_candidate] = numba_distance_function(
                    q[i_channel],
                    X[
                        i_channel,
        ...
        if mask[i_candidate]:
            for i_channel in range(n_channels):
                ...
                _C = z_normalize_series_with_mean_std(
                    ...
                    X_means[i_channel, i_candidate],
                    X_stds[i_channel, i_candidate],
                )
                distance_profile[i_channel, i_candidate] = numba_distance_function(
                    q[i_channel], _C
                )
