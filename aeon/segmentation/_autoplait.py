...
SIGMOD 2014.
https://doi.org/10.1145/2588555.2588556
"""
...
class AutoPlaitSegmenter(BaseSegmenter):
    ...
    _tags = {
        "returns_dense": True,
        "capability:univariate": True,  # Fixed: Changed to True
    ...
    def _compute_total_mdl(self):
        ...
        Implements Equation 6 from [1], using recursive universal code length.
        Cost_T = Cost(M) + Cost(X|M)
        ...
        costT = (
            cost + log_star(r) + log_star(m) + m * log_2(r) + (4 * 8) * r * r
...
class _Regime:
    ...
    def estimate_hmm_k(self, k: int) -> None:
        ...
        # Initial delta heuristic, usually overwritten by _update_deltas in main loop
    ...
    def mdl(self) -> float:
        ...
        Uses log_star for the model cost, as defined in the paper.
        ...
        costM = FLOATING_POINT_CONSTANT * (k + k * k + 2 * k * d) + log_star(k)
...
class _CPS:
    ...
    def _search_aux(self, start_idx, length, regime0, regime1):
        ...
        Implements the trellis diagram search described in
        Equations 8 and 9 [cite: 385, 398-403].
        ...
        # Handle degenerate case early: Removed raise error for delta=0
        ...
        # Ensure d0 and d1 are clamped to avoid log(0)
        d0 = max(EPSILON, min(d0, 1.0 - EPSILON))
        d1 = max(EPSILON, min(d1, 1.0 - EPSILON))
    ...
    def _update_deltas(self, regime0, regime1):
        """
        Update regime transition probabilities (delta) based on switch counts.

        [cite_start]Implements Equation 14 from [cite: 446-448].
        delta_12 = N_12 / sum(|s_1|)
        ...
        if len0 > 0:
            regime0.delta = max(EPSILON, min(float(n_01) / float(len0), 1.0 - EPSILON))
        else:
            regime0.delta = EPSILON

        # Do the same for regime 1
        len1 = regime1.total_length
        n_10 = regime1.num_segments
        if len1 > 0:
            regime1.delta = max(EPSILON, min(float(n_10) / float(len1), 1.0 - EPSILON))
        else:
            regime1.delta = EPSILON

    def _estimate_regimes_aux(self, current_regime, regime0, regime1):
        """
        Estimate regime parameters by iterative refinement.

        [cite_start]Implements Algorithm 2 (RegimeSplit)[cite: 431].
        Loop Order:
        1. [cite_start]CutPointSearch (Find Segments) [cite: 428]
        2. [cite_start]BaumWelch (Update Parameters) [cite: 429]
        3. [cite_start]Update Regime Transitions (Update Delta) [cite: 430]
        ...
        # Initialize with current best state
        opt0.copy_to(regime0)
        opt1.copy_to(regime1)

        for i in range(self.ap_seg.max_infer_iter):
            # Phase 1: Find cut-points using CURRENT params (Algorithm 2, Line 7)
            ...
            # Phase 2: Update Model Parameters (Algorithm 2, Lines 8-9)
            # This estimates Theta based on the segments found in Phase 1
            regime0.estimate_parameters()
            regime1.estimate_parameters()

            # Phase 3: Update Regime Transitions explicitly (Algorithm 2, Line 10)
            self._update_deltas(regime0, regime1)
...
def backward(hmm, m, beta, scale, emission_probs):
    ...
    for t in range(m - 2, -1, -1):
        ...
        # Normalize by scale[t] - FIXED: added EPSILON to prevent overflow
        beta[t] /= scale[t] + EPSILON
...
def log_star(x):
    """
    Compute universal code length for integer x.

    Implements log*(x) = log2(x) + log2(log2(x)) + ...
    ...
    cur = float(x)
    while cur > 0:
        cur = math.log(cur, 2.0)
        if cur > 0:
            cost += cur
        else:
            break
    # The paper also adds a constant to the recursive sum,
    # usually approx 2.865 for the base case, but standard
    # MDL implementations often just sum the log terms.
    return cost + 2.865
