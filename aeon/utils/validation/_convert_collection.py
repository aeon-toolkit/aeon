...
def _nested_univ_is_equal(X):
    ...
    This function checks if all series in a nested DataFrame have the same length. It assumes that series are of equal length over channels, so it only tests the first channel.
