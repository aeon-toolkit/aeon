...
def _check_python_version(obj, package=None, msg=None, severity="error"):
    ...
    Parameters
    ----------
    obj : aeon estimator, BaseEstimator descendant
...
def _check_estimator_deps(obj, msg=None, severity="error"):
    ...
    Checks whether dependency requirements of `BaseEstimator`-s in `obj`
    are satisfied by the current python environment.

    Parameters
    ----------
    obj : `aeon` object, `BaseEstimator` descendant, or list/tuple thereof
    ...
    compatible : bool, whether `obj` is compatible with python environment
        ...
        if `obj` contains multiple `BaseEstimator`-s, checks whether all are compatible
