...
def test_naive_forecaster_model_with_regressor_pyfunc_output(
    ...
    mlflow_aeon.save_model(
        forecaster=naive_forecaster_model_with_regressor, path=model_path
    )
    loaded_pyfunc = mlflow_aeon.pyfunc.load_model(model_uri=model_path)
...
def test_signature_and_example_for_pyfunc_predict(
    ...
    mlflow_aeon.save_model(forecaster=auto_arima_model, path=model_path_primary)
...
def test_load_from_remote_uri_succeeds(auto_arima_model, model_path, mock_s3_bucket):
    ...
    mlflow_aeon.save_model(forecaster=auto_arima_model, path=model_path)
    ...
    reloaded_forecaster = mlflow_aeon.load_model(model_uri=model_uri)

    np.testing.assert_array_equal(
        auto_arima_model.predict(),
        reloaded_forecaster.predict(),
    )
