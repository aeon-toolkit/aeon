...
def msm_distance(
    ...
    if x.ndim == 3 and y.ndim == 3:
        ...
        for curr_x, curr_y in zip(x, y):
            distance += _msm_distance(curr_x, curr_y, bounding_matrix, independent, c)
        return distance
    ...
    if x.ndim == 3 and y.ndim == 3:
        ...
        for curr_x, curr_y in zip(x, y):
            if independent:
                cost_matrix = np.add(
                    cost_matrix,
                    _msm_independent_cost_matrix(curr_x, curr_y, bounding_matrix, c),
                )
            else:
                cost_matrix = np.add(
                    cost_matrix,
                    _msm_dependent_cost_matrix(curr_x, curr_y, bounding_matrix, c),
                )
        return cost_matrix
    raise ValueError("x and y must be 1D, 2D, or 3D arrays")
