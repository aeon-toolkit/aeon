...
from aeon.utils.conversion._convert_collection_to_numba_list import (
...
def soft_msm_distance(
    ...
    Soft-MSM is a differentiable relaxation of the move-split-merge (MSM)
    distance that replaces the hard minimum over alignment paths with a
    temperature-controlled soft minimum. When :math:`\gamma \rightarrow 0`,
    soft-MSM recovers the standard MSM distance.
    ...
    gamma : float, default=1.0
        ...
        Smaller values recover the hard-min behaviour (MSM), larger values
        produce smoother alignments.

    Returns
    -------
    float
        soft-MSM distance between ``x`` and ``y``, minimum value 0.
...
def soft_msm_alignment_matrix(
    ...
    r"""Compute the soft-MSM alignment (occupancy) matrix between two series.

    The alignment matrix encodes the expected contribution of each diagonal
    match edge under the soft-MSM alignment distribution, together with the
    soft-MSM distance.
    ...
    -------
    np.ndarray
        Alignment (occupancy) matrix of shape
        ``(n_timepoints_x, n_timepoints_y)``.
    float
        The soft-MSM distance between the two time series.
    """
    if x.ndim == 1 and y.ndim == 1:
...
def soft_msm_grad_x(
    ...
    """
    Gradient (Jacobian) of soft-MSM distance w.r.t. the first series ``x``.

    Returns ``(dx, distance)`` with ``dx`` giving the derivative of the
    soft-MSM distance with respect to ``x``.
    ...
    gamma : float, default=1.0
        Controls the smoothness of the soft minimum over alignment paths.
        Must be strictly positive for differentiability.
    window : float or None, default=None
        The window to use for the bounding matrix. If None, no bounding matrix
        is used.
    itakura_max_slope : float or None, default=None
    ...
    -------
    dx : np.ndarray
        The gradient of the soft-MSM distance with respect to ``x``.
        If ``x`` is univariate (``x.ndim == 1``), the shape is
        ``(n_timepoints,)``. If ``x`` is multivariate (``x.ndim == 2``),
        the shape is ``(n_channels, n_timepoints)``.
    distance : float
        The soft-MSM distance between the two time series.
    ...
    ------
    ValueError
        If ``gamma`` is not strictly positive.
    """
    if gamma <= 0:
