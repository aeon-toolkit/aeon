...
def _additional_distance_tests(x, y, pairwise_distance, single_to_multiple_distance,
    ...
    for i in range(pairwise_distance.shape[0]):
        for j in range(pairwise_distance.shape[0]):
            ...
            np.testing.assert_almost_equal(pairwise_distance[i, j],
                                           distance_function(x[i], x[j]))

    for i in range(single_to_multiple_distance.shape[0]):
        np.testing.assert_almost_equal(single_to_multiple_distance[i],
                                       distance_function(x[0], y[i]))
...
def test_dtw_distance():
    np.testing.assert_almost_equal(dtw_distance(x_multivariate, y_multivariate),
                                   4408.25)
    np.testing.assert_almost_equal(
        dtw_distance(x_multivariate, y_multivariate, window=0.1),
        4408.25)
...
def test_ddtw_distance():
    ...
    np.testing.assert_almost_equal(wddtw_distance(x_univariate, y_univariate),
                                   199.53608069124422)
    np.testing.assert_almost_equal(ddtw_distance(x_multivariate, y_multivariate),
                                   3833.84375)
    np.testing.assert_almost_equal(
        ddtw_distance(x_multivariate, y_multivariate, window=0.1),
        3833.84375)
...
def test_wdtw_distance():
    np.testing.assert_almost_equal(wdtw_distance(x_multivariate, y_multivariate),
                                   1930.035439970180)
    np.testing.assert_almost_equal(
        wdtw_distance(x_multivariate, y_multivariate, window=0.1),
        1930.035439970180)
...
def test_wddtw_distance():
    np.testing.assert_almost_equal(wddtw_distance(x_multivariate, y_multivariate),
                                   1725.86611586604)
    np.testing.assert_almost_equal(
        wddtw_distance(x_multivariate, y_multivariate, window=0.1),
        1725.86611586604)
...
def test_erp_distance():
    np.testing.assert_almost_equal(erp_distance(x_multivariate, y_multivariate),
                                   4408.25)
    np.testing.assert_almost_equal(
...
def test_msm_distance():
    ...
    np.testing.assert_almost_equal(
        msm_distance(x_multivariate, y_multivariate, independent=False),
        164.0)
    np.testing.assert_almost_equal(
...
def test_twe_distance():
    ...
    np.testing.assert_almost_equal(
        twe_distance(x_multivariate, y_multivariate, window=0.1),
        1501.001)
