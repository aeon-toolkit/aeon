...
class RISTRegressor(BaseRIST, BaseRegressor):
    ...
    This regressor is a hybrid pipeline using the ``RandomIntervalTransformer`` with
    ``Catch22`` features and summary stats, and the ``RandomDilatedShapeletTransformer``.
    Both transforms extract features from different series transformations:
    ``1st Order Differences``, ``PeriodogramTransformer``, and ``ARCoefficientTransformer``.
    ...
    n_intervals : ``int``, callable, or ``None``, default=``None``
        ...
        a function that takes a 3D ``np.ndarray`` input and returns an ``int``.
        Functions may extract a different number of intervals per ``series_transformer`` output.
    ...
    n_shapelets : ``int``, callable, or ``None``, default=``None``
        ...
        a function that takes a 3D ``np.ndarray`` input and returns an ``int``.
        Functions may extract a different number of shapelets per ``series_transformer`` output.
    ...
    series_transformers : ``TransformerMixin``, ``list``, ``tuple``, or ``None``, default=``None``
        ...
        Differences, PeriodogramTransformer, ARCoefficientTransformer]``.

        A ``list`` or ``tuple`` of transformers will extract intervals from
        all transformations and concatenate the output. Including ``None`` in the list
        or tuple will use the series as is for interval extraction.
    ...
    estimator : ``sklearn`` regressor, default=``None``
        ...
        Defaults to an ``ExtraTreesRegressor`` with 200 trees.
