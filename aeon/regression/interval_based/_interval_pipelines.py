...
class RandomIntervalRegressor(BaseRegressor):
    ...
    dilation : ``int``, ``list`` or ``None``, default=``None``
        ...
        list of ``int``, a random dilation value is selected from the list for each
        interval.

        The dilation value is selected after the interval start and end points. If the
        number of values in the dilated interval is less than ``min_interval_length``,
        the amount of dilation applied is reduced.
    estimator : ``sklearn`` regressor, optional, default=``None``
        A ``sklearn`` estimator to be built using the transformed data.
