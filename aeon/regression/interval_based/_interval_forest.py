...
class IntervalForestRegressor(BaseIntervalForest, BaseRegressor):
   """Configurable interval extracting forest regressor.
    ...
    base regressor on summary statistics extracted from each interval. Forms an
    ...
    By default, uses a configuration similar to ``TimeSeriesForest`` [1].
    ...
    n_intervals : ``int``, ``str``, ``list`` or ``tuple``, default=``"sqrt"``
        ...
        A ``list`` or ``tuple`` of ``int`` and/or ``str`` will extract the number of intervals using
    ...
    series_transformers : ``BaseTransformer``, ``list``, ``tuple``, or ``None``, default=``None``
        ...
        all transformations and concatenate the output. Including ``None`` in the ``list`` or ``tuple``
    ...
    att_subsample_size : ``int``, ``float``, ``list``, ``tuple`` or ``None``, default=``None``
        ...
        using a ``list`` or ``tuple``. Any ``list`` or ``tuple`` input must be the same length as
        the number of ``series_transformers``.
    replace_nan : ``"nan"``, ``int``, ``float`` or ``None``, default=``None``
        The value to replace ``NaN`` and infinite values with before fitting the base
        estimator. ``int`` or ``float`` input will replace with the specified value, while
        ``"nan"`` will replace infinite values with ``NaN``. If ``None``, do not replace ``NaN``.
    ...
    parallel_backend : ``str``, ``ParallelBackendBase`` instance or ``None``, default=``None``
        Specify the parallelization backend implementation in ``joblib``. If ``None``, a ``prefer``
