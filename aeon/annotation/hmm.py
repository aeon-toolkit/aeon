class HMM(BaseSeriesAnnotator):
    """
    Implements a simple Hidden Markov Model (HMM) fitted with Viterbi algorithm.

    The HMM annotation estimator uses the Viterbi algorithm to fit a sequence of 
    'hidden state' class annotations (represented by an array of integers the same size
    as the observation) to a sequence of observations.

    Parameters
    ----------
    emission_funcs : list
        List of callables for calculating the emission probabilities.
    transition_prob_mat : np.ndarray
        Matrix of transition probabilities from hidden state to hidden state.
    initial_probs : np.ndarray, optional
        Probability over the hidden state identity of the first state.
    ...
    emission_funcs : list
        Functions for calculating the emission probabilities.
    transition_prob_mat : np.ndarray
        Matrix of transition probabilities from hidden state to hidden state.
    """
    ...
    def _calculate_trans_mats(
        ...
        """
        Calculate the transition matrices used in the Viterbi algorithm.
        ...
        initial_probs : np.ndarray
            Initial probabilities for each hidden state.
        emi_probs : np.ndarray
            Emission probabilities for each observation.
        transition_prob_mat : np.ndarray
            Matrix of transition probabilities from hidden state to hidden state.
        num_obs : int
        ...
        -------
        Tuple[np.array, np.array]
            Transition probability matrix and transition id matrix.
        """
    ...
    def _make_emission_probs(
        ...
        """
        Calculate the probability each observation comes from each hidden state.
        ...
        -------
        np.ndarray
            Matrix of emission probabilities for each observation.
        """
